<div align="center">

# 👋 Hi, I'm Mansi Garg

### 💻 Data Science Student | 🐍 Python Developer | 🤖 AI Enthusiast

<p>
  <a href="https://github.com/mansi-garg27">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
  <a href="https://www.linkedin.com/in/mansi-garg-898138344">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
</p>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=1000&color=00BFFF&center=true&vCenter=true&width=600&lines=Data+Science+Student;Python+Developer;AI+Enthusiast;Building+Real-World+Projects;Always+Learning+%26+Improving" />

</div>

---

## 👩‍💻 About Me

🎓 I'm a **B.Tech CSE (Data Science)** student passionate about **Data Science, Artificial Intelligence and Software Development**.

🐍 I enjoy building practical projects using **Python, SQL, Java and Power BI**.

📊 I love working with data, finding patterns and turning raw information into meaningful insights.

🤖 Currently exploring **Machine Learning, AI and advanced Data Analytics**.

🚀 My goal is to become a strong, industry-ready **Data Science professional** by building real-world projects and continuously improving my problem-solving skills.

---

## 🛠️ Tech Stack

### 💻 Programming Languages

<p>
  <img src="https://skillicons.dev/icons?i=python,c,java" />
</p>

### 📊 Data Science & Analytics

<p>
  <img src="https://skillicons.dev/icons?i=mysql" />
</p>

<p>
  <img src="https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white"/>
  <img src="https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white"/>
  <img src="https://img.shields.io/badge/Matplotlib-11557C?style=for-the-badge&logo=matplotlib&logoColor=white"/>
  <img src="https://img.shields.io/badge/Plotly-3F4F75?style=for-the-badge&logo=plotly&logoColor=white"/>
  <img src="https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black"/>
</p>

### ⚙️ Tools & Technologies

<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode" />
</p>

---

## 🚀 Featured Projects

### 🛡️ SAP ResiliX

**Resilient Supply Chain Risk Monitoring Platform**

A project designed to monitor supply-chain risks and help businesses prepare for potential disruptions.

**Tech Stack:**

`React` `FastAPI` `Python` `Database` `JWT Authentication`

---

### 📈 Sales Tracker Dashboard

Interactive sales analytics dashboard for exploring and visualizing business data.

**Tech Stack:**

`Python` `Plotly` `Dash` `SQLite`

---

### 📊 MySQL Sales Data Analysis

Performed sales-data analysis using SQL to extract useful business insights.

**Concepts Used:**

`JOIN` `GROUP BY` `HAVING` `Aggregate Functions`

---

### 📉 Power BI Sales Dashboard

Interactive dashboard created to visualize sales performance and business metrics.

**Tech Stack:**

`Power BI` `Data Visualization`

---

### 💰 Expense Tracker

A menu-driven Python application for managing personal expenses.

**Features:**

`Add` `View` `Update` `Delete`

**Concepts Used:**

`Python` `Functions` `File Handling` `CRUD Operations`

---

### 🔐 Random Password Generator

A Python-based application for generating secure random passwords.

**Tech Stack:**

`Python` `Randomization` `String Manipulation`

---

## 📚 Currently Learning

<p align="center">

<img src="https://img.shields.io/badge/DSA-FF6F00?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Machine%20Learning-8E44AD?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Advanced%20SQL-00758F?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Data%20Analytics-1F77B4?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Artificial%20Intelligence-00A67E?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black"/>

</p>

---

## 🎯 2026 Goals

- 🚀 Become placement-ready
- 🧠 Strengthen DSA & problem solving
- 📊 Build advanced Data Science projects
- 🤖 Learn Machine Learning & AI
- 💻 Improve Python & SQL skills
- 🌐 Build impactful real-world projects
- 🤝 Contribute to open-source projects

---

## 📊 GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=mansi-garg27&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" height="170"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=mansi-garg27&layout=compact&theme=tokyonight&hide_border=true" height="170"/>

</div>

---

## 🔥 GitHub Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=mansi-garg27&theme=tokyonight&hide_border=true"/>

</div>

---

## 📈 My Coding Journey

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=mansi-garg27&theme=tokyo-night&hide_border=true"/>

</div>

---

## 💡 What I Like Building

<div align="center">

| 📊 Data Analytics | 🤖 AI & ML |
|:---:|:---:|
| Business Intelligence | Machine Learning |
| Data Visualization | AI Applications |
| SQL Solutions | Intelligent Systems |

| 🐍 Python Development | 🌐 Software Development |
|:---:|:---:|
| Automation Tools | Full-Stack Applications |
| Data Applications | Real-World Solutions |

</div>

---

## 💻 Coding Profiles

<div align="center">

<a href="https://leetcode.com/u/mansigarg27/">
<img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black"/>
</a>

<a href="https://www.hackerrank.com/">
<img src="https://img.shields.io/badge/HackerRank-00EA64?style=for-the-badge&logo=hackerrank&logoColor=black"/>
</a>

</div>

---

## 🤝 Let's Connect

<div align="center">

I'm always interested in **learning, collaborating and building useful projects**.

<br>

<a href="https://www.linkedin.com/in/mansi-garg-898138344">
<img src="https://img.shields.io/badge/LinkedIn-Mansi%20Garg-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://github.com/mansi-garg27">
<img src="https://img.shields.io/badge/GitHub-Mansi%20Garg-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

</div>

---

<div align="center">

### ⭐ Thanks for visiting my profile!

<img src="https://komarev.com/ghpvc/?username=mansi-garg27&label=Profile%20Views&color=blue&style=flat"/>

</div>
